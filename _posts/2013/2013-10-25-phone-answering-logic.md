...
1. server, worker_cli, user_cli。两个客户端worker_cli，user_cli分别接收客服和客户的请求，并发送给服务器，服务器处理请求，可能需要调用下层的功能，并回复结果给客户端，客户端显示结果。
...
1. `客服IP -> workerid`。这张表是不会变的，这张表可以判断进来的连接是否是客服，顺便存了下`workerid`。
2. `workerid -> workerfd`。每连接一个客服，先查表1，得到`workerid`，然后存下对应的描述子`workerfd`。这张表需要是因为系统在`answer_call`后要建立`clientfd`和`workerfd`的对应关系，这样客服和客户才能互相通话，`clientfd`已知，但是`answer_call`返回的是`workerid`，所以必须有这么一张表。
3. `workerfd -> workerid` 。用于客服请求离开和请求回来。
4. `clientfd -> workerfd`。用于客户向客服发消息。
5. `workerfd -> clientfd`。用户客服向客户发消息。

经过一段时间的通话，客服会请求通话完毕或者客户挂机，这时服务器会关闭与客户的连接，而不断开客服的连接，只是把客服放到空闲列表去。一分钟的通话时限可以由客服自己把握。

####异常情况

1. 客服断开连接。因为这是个模拟电话的系统，电话的话，分机和总机是一直连着的， 那也就意味着，客服和服务器的连接永远不会断，除非出故障了。所以这时系统应该报告故障，同时把那个客服置为忙。服务器可以有个功能，显示当前已连接的客服，这样就可以知道哪些分机坏了。
2. 客服离开，但没有事先请求离开。然后他被分配到去接待某个客户。客户发了消息，就会一直得不到回复。这个异常可以不处理，因为客户是知道客服id的，他可以投诉这个客服。
3. 客服发送消息，但是并没有客户与他连接。这时服务器回复客服，而不帮他转发。
4. 拒绝服务攻击。因为这是个电话模拟系统，所以跟网络里的攻击还不太一样。使用select后，当一个套接字是可以读了，服务器就去处理那个套接字了，但是你不知道该读多少字节，可以用换行符表示一次读的截止，如果是阻塞读，那么就可能有恶意用户发送一个字节，然后去睡眠，那么服务器就阻塞在`readline`里。假设有一个恶意用户，发起连接，那么系统会分配一个客服给他，假设他挂机，系统会收回那个客服，假设他不挂机，客服发现客户长时间不说话，他可以挂机，即发送`worker_back`消息。这时系统可以把`clientfd`和`workerfd`socket对从表4，5中删除。恶意用户可以不停的发起连接，然后直接挂机。那么系统会不断的分配，收回。但是系统不会阻塞，客服不会因为恶意用户而一直忙，那么假设恶意用户不直接挂机，在那待一会。  
...
1. 客户断开连接，这个由从客户套接字`Read`到0检测出。系统调用`worker_back`把服务它的客服放回空闲列表。
2. 客服断开连接，由客服套接字`Read`到0检测出。
...
1. 把客服的离开、返回、通话完毕的消息转发给服务器。
2. 把客服给客户的消息发送给服务器，以及把客户经由服务器发送的消息显示给客服。
...
###系统测试
####应答逻辑测试
`PriorityQueue`由`TestPriorityQueue`测试，应答逻辑类`PhoneAnswer`由`TestPhoneAnswer`测试。
####服务器客户端测试
为测试方便，客服尽量少，2个的话测试不了分配客服的公平性，最少3个。截图`test_server_client.png`表明了测试过程。
截图分了8个窗口，第1行第1个是服务器，剩下3是客服0-2，第二行全是客户。测试按以下步骤进行。

1. 服务器开启，3个客服连接。
2. 客服0发送消息。异常情况3。
3. 用户0连接。被分配到客服0。
4. 用户0发送消息给客服。
5. 客服0回信息，但是忘记在前面加上2表示是发消息，系统打印输入格式给他看。
6. 客服0正确回信息。
7. 客服0请求离开。在通话过程中，请求被拒绝。
8. 用户0觉得回答很满意，回复谢谢后，挂机，客服0重新空闲。工作时间为1，要在客服1，2被分配后才会再次被分配到用户。
9. 客服0再次请求离开。请求被允许。
10. 用户0连接。客服1被分配。
11. 客服0回来。插在客服2后面。
12. 用户1连接。客服2被分配。
13. 用户2连接。客服0被分配。
14. 用户3连接。没有空闲客服，于是用户3不停的尝试。
15. 用户0挂机。客服1重新空闲，用户3的下一次连接将被接受。

为了方便调试，服务器鉴定连接的是否是客服的方式是看连接的顺序，所以如果在3个客服连接前有客户连接了，他会被认定为是客服，这不属于异常情况。因为实际中你鉴定是否是客服可以用进来的连接的IP确定，这样就不受连接顺序影响了。

####清零测试
由于工作时间的可能上溢出，所以设了个长度为24小时的`alarm(24*60*60)`信号（测试时用10s)，在处理函数对所有客服的工作时间清零，主要测试的是信号会中断系统调用，由于系统Read和Writen都不可能阻塞，所以测中断它们很难，（Writen有重启功能，而Read没有），但是测`select`应对中断好测，并且果然出错了。因为《Unix Network Programming》源代码的包装函数`Select`并没有重启功能，返回值小于零就直接退出了，于是改它的`Select`，在`errno=EINTR`时再调用`Select`。改后在测试时暂时没有测出错误，测试情况见截图test_clear_time。

    while ( (n = select(nfds, readfds, writefds, exceptfds, timeout)) < 0) {
        if (errno != EINTR) err_sys("select error");
    }

###调试调出的错误总结
- 类成员函数前没加类限定符。
- read函数不会自动添加`'\0'`，所以服务器应该是读到多少发送多少，而不是`Writen(sockfd, buf, strlen(buf))`。
- 在客户的客户端中，`connect`后接收到系统忙的信息，睡10秒后再`connect`，显示`Already connect`，需要在`connect`前把套接字关闭再生成一个新的套接字，用它再去连。

        for ( ; ; ) {
                sockfd = Socket(AF_INET, SOCK_STREAM, 0);
                Connect(sockfd, (SA *) &servaddr, sizeof(servaddr));
                n = Read(sockfd, buf, MAXLINE);
                if (strncmp(buf, conn_reply_start, strlen(conn_reply_start)) != 0) {
                    /* system busy */
                    Write(fileno(stdout), conn_rej_msg, strlen(conn_rej_msg));
                    Close(sockfd);
                    sleep(10);
                    continue;
                }

- 改代码后，再启动server，显示`bind error: Address already in use`，这个问题的话，过30秒左右再运行就好了。