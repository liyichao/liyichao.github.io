...
客户和客服谁先发消息不一定，两个客户端从标准输入获得消息，然后发送给对面的，同时它们也要看有没有消息从套接字来，所以它们也需要`IO`多路复用。客户可以断开连接，这时该把`clientfd`从`select`的`descriptor set`中移除，同时通知接待客户的客服，然后更新表4和表5。
