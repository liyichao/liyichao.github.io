...
###服务器与客服及客户的通信
总共20个客服，意味着并发连接只有40个，考虑用`select`实现服务器，也较少移植性问题，如果以后系统扩展了，可以换为`kqueue`或`epoll`。有时间也可以用线程或进程实现看看。
...
- 3：client send。客户发送消息，服务器查表转发。注意，检查是否有空闲客服的操作放在了客户连接建立的时候，因为客户连接，就代表他想通话，系统就分配客服给他，这也意味着，客户连接后直接调用`readline`，如果读到服务器的拒绝信息，那么就该切断连接，过会再连。
