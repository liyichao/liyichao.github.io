...
综上，Spark 程序是通过启动 driver 和 executor 跑起来的，driver 把 Spark 程序生成的 DAG 规划为 Stage 和 Task，然后通过 RPC 使 executor 跑 Task，从而完成 Spark 程序的执行，而 driver，executor 都是通过 spark-submit 往 Spark RestServer 提交任务，然后通过资源管理器（Yarn, Mesos, Spark standlone）跑起来的。
